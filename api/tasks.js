...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      const { data, error } = await supabase.from("accountability_tasks").select("*").order("due_at", { ascending: true });
    ...
    if (req.method === "POST") {
      ...
      const { data, error } = await supabase.from("accountability_tasks").insert(task).select("*").single();
      if (error) throw error;
    ...
    if (req.method === "PATCH") {
      ...
      const { data, error } = await supabase.from("accountability_tasks").update(task).eq("id", id).select("*").single();
      if (error) throw error;
